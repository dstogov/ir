...
int ir_compute_live_ranges(ir_ctx *ctx)
{
	...
	for (b = ctx->cfg_blocks_count; b > 0; b--) {
				...
				for (j = 0; j < use_list->count; j++) {
					...
					if (insn->op == IR_PHI) {
						if (insn->ops[k] > 0) {
							...
							ir_add_live_range(ctx, ctx->vregs[insn->ops[k]], insn->type, bb->start, bb->end);
						}
					}
				}
			}
		...
		IR_BITSET_FOREACH(live, len, i) {
			...
			ir_add_live_range(ctx, i, IR_VOID, bb->start, bb->end);
...
//			if (flags & IR_OP_FLAG_DATA) {
			if ((flags & IR_OP_FLAG_DATA) || ((flags & IR_OP_FLAG_MEM) && insn->type != IR_VOID)) {
				if (ctx->vregs[i] && ir_bitset_in(live, ctx->vregs[i])) {
					if (1 || insn->op != IR_PHI) { // TODO: Should Phi start live-range from block start ???
						/* intervals[opd].setFrom(op.id) */
						ir_fix_live_range(ctx, ctx->vregs[i], bb->start, i);
					}
					/* live.remove(opd) */
				...
				for (j = 1; j <= n; j++) {
					if (IR_OPND_KIND(flags, j) == IR_OPND_DATA) {
						...
						if (input > 0 && ctx->vregs[input]) {
							...
							ir_add_live_range(ctx, ctx->vregs[input], ctx->ir_base[input].type, bb->start, i);
							/* live.add(opd) */
...
#if 0
		/* for each phi function phi of b */
		if (bb->predecessors_count > 1) {
			...
			for (j = 0; j < use_list->count; j++) {
				...
				if (insn->op == IR_PHI) {
					...
					IR_ASSERT(ctx->vregs[use]);
					ir_bitset_excl(live, ctx->vregs[use]);
				}
			}
		}
#endif
		...
		if ((bb->flags & IR_BB_LOOP_HEADER)
		 && !ir_bitset_empty(live, len)) {
			...
			do {
				...
				IR_BITSET_FOREACH(live, len, i) {
					ir_add_live_range(ctx, i, IR_VOID, child_bb->start, child_bb->end);
				} IR_BITSET_FOREACH_END();
...
static int ir_linear_scan(ir_ctx *ctx)
{
	...
	ir_bitset active, inactive, handled;
	ir_live_interval *ival;
	int current, i;
	uint32_t len;
	ir_ref position;
	...
	len = ir_bitset_len(ctx->vregs_count + 1);
	active = ir_bitset_malloc(ctx->vregs_count + 1);
	inactive = ir_bitset_malloc(ctx->vregs_count + 1);
	handled = ir_bitset_malloc(ctx->vregs_count + 1);
